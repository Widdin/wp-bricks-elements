	...
	wbeGalleryLazyImageObserver = new IntersectionObserver(function (entries, observer) {
		entries.forEach(function (entry) {
			if (entry.isIntersecting) {
				...
				if (lazyImage.classList.contains('wbe-gallery__lightbox')) {
					lazyImage.addEventListener('click', openLightbox, { passive: false });
				}
			}
		});
	});
...
function openLightbox(e) {
	...
	const aspectRatio = eTarget.style.aspectRatio.split('/');

	items = [
		{
			src: getHighestResImg(eTarget),
			w: aspectRatio[0].trim(),
			h: aspectRatio[1].trim(),
		},
	];
...
function wbeGallerySendAJAX() {
	...
	galleryImages.forEach(function (e) {
		e.classList.add('wbe-gallery__fadeOut');
	});
...
function wbeGalleryUpdateObserver() {
	...
	lazyImages.forEach(function (lazyImage) {
		wbeGalleryLazyImageObserver.observe(lazyImage);
	});
}
